...
字符串操作在我们编程中是经常会接触到的，正则表达式确是是一把操作字符串的利器，我们今天就来罗列一下js正则相关的api。

## 创建

js中提供了字面量和调用RegExp构造函数两种方式来创建正则表达式实例。在ES3中使用使用字面量方式创建的正则表达式会共用同一个RegExp实例，在ES5后就没有这一问题了(IE9+);
```
// 字面量写法
